...
- 코치를 입력 받고 파싱 후 유효성 검사를 받는다.
  - 코치 이름들은 `,`로 구분하여 파싱한다.
  - 코치는 최소 2명, 최대 5명이다.
  - 코치 이름은 비어있을 수 없다.
  - 코치 이름은 2~4글자이다.
  - 코치 이름은 중복될 수 없다.
- 각 코치가 먹지 못하는 음식을 입력 받고 파싱 후 유효성 검사를 받는다.
  - 먹지 못하는 음식은 `,`로 구분하여 파싱한다.
    - 먹지 못하는 음식이 없는 경우도 고려한다.
  - 먹지 못하는 음식은 0~2개 이다.
  - 먹지 못하는 음식은 비어있을 수 없다.
  - 먹지 못하는 음식은 정해진 목록에 존재하는 값이어야 한다.
  - 먹지 못하는 음식은 중복될 수 없다.
...
- 프로그램 시작 문구 출력
- 메뉴 추천 결과 출력
  - 구분, 카테고리, 각 코치가 추천한 메뉴를 표 형식으로 출력
  - 추천 완료 문구 출력
- 에러 메시지
